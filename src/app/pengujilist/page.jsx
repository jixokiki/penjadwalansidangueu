...
export default function DosenListPenguji() {
  ...
  const fetchDosen = async () => {
    try {
      ...
      const dosenList = usersSnapshot.docs
        ...
        .filter((user) => user.role === "penguji"); // Filter for dosen role
      setDosen(dosenList);
    } catch (error) {
  ...
  return (
    ...
    <div className={styles.container}>
      <h1 className={styles.title}>List of Penguji</h1>
...
export function DosenListPenguji2() {
  const [dosen, setDosen] = useState([]);
  const [error, setError] = useState(null);
